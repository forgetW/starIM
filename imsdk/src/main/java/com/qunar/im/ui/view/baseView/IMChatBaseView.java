...
import com.qunar.im.ui.util.CornerTransform;
import com.qunar.im.ui.util.ResourceUtils;
import com.qunar.im.ui.view.IconView;
import com.qunar.im.ui.view.TextDrawable;
...
public class IMChatBaseView extends RelativeLayout implements IMessageItem {
    ...
    protected TextView mLeftNickName, statusView, readStateText;
    ...
    protected void findViewById() {
        ...
        readStateText = findViewById(R.id.read_state_text);
...
//        mRightChatView.setBackgroundResource(R.drawable.atom_ui_balloon_right);
    ...
    public void setReadStateShow(boolean show) {
        if (show) {
            readStateText.setVisibility(VISIBLE);
        } else {
            readStateText.setVisibility(GONE);
        }
        send_states_text.setVisibility(GONE); //xu
//        if (show) {   //xu
...
//        setReadStateShow(showNick);
    ...
    public void initFontSize() {
        ...
        mTimeTextView.setTextSize(TypedValue.COMPLEX_UNIT_PX,
                context.getResources().getDimensionPixelSize(R.dimen.atom_ui_text_size_extra_11)
                        + ResourceUtils.getFontSizeIntervalPX(context));
    }
...
//            p.processSendStatesView(send_states_text, this);//xu
            p.processSendStatesView(readStateText, this);//xu
                        ...
                        ((Activity) context).runOnUiThread(new Runnable() {
                            ...
                            public void run() {
                                if (nick != null) {
                                    ...
                                    showImage(nick.getHeaderSrc(), mRightRoundedImageView, nick.getName());
//                                    gravatarHandler.requestGravatarEvent(nick.getXmppId(), nick.getHeaderSrc(), mRightRoundedImageView);

                                } else {
                                    showImage(defaultHeadUrl, mRightRoundedImageView, "");//新加
//                                    showImage(defaultHeadUrl, mRightRoundedImageView, nick.getName()); //old
                            ...
                            ((Activity) context).runOnUiThread(new Runnable() {
                                ...
                                public void run() {
                                    if (nick != null) {
                                        message.setNick(nick);
                                        //只是绘制左上角和右上角圆角
                                        showImage(nick.getHeaderSrc(), mLeftRoundedImageView, nick.getName()); //新加
//                                        gravatarHandler.requestGravatarEvent(nick.getXmppId(), TextUtils.isEmpty(nick.getHeaderSrc()) ? defaultHeadUrl : nick.getHeaderSrc(), mLeftRoundedImageView);
                                        ...
                                        //只是绘制左上角和右上角圆角
                                        showImage(defaultHeadUrl, mLeftRoundedImageView, "");//新加
...
//                gravatarHandler.requestGravatarEvent(nickOrUid, mLeftRoundedImageView);
    ...
    private void showImage(String imageUrl, SimpleDraweeView imageView, String name) {
        int w = getResources().getDimensionPixelSize(R.dimen.atom_ui_image_mid_size);
        int h = getResources().getDimensionPixelSize(R.dimen.atom_ui_image_mid_size);
        double fontSize, size = 30;
        if (Pattern.matches("(^[a-zA-Z ]+$)", name)) {
            if (name.contains(" ")) {
                name = name.split(" ")[0];
            }
            if (name.length() == 1) {
                fontSize = size / name.length() / 1.5;
            } else if (name.length() == 2) {
                fontSize = size / name.length() * 1.1;
            } else {
                name = name.substring(0, 3);
                fontSize = size / name.length() * 1.4;
            }
        } else {
            if (name.length() == 3) {
            ...
            fontSize = size / name.length() / 1.6;
            if (name.length() == 1) {
                fontSize = size / name.length() / 2;
            }
        }
        CornerTransform transformation = new CornerTransform(context, Utils.dpToPx(context, 6));
        transformation.setExceptCorner(false, false, false, false);

        TextDrawable textDrawable = TextDrawable.builder()
                .beginConfig()
                .bold()
                .width(Utils.dpToPx(context, w))
                .height(Utils.dpToPx(context, h))
                .fontSize(Utils.dpToPx(context, 38))
                .endConfig()
                .buildRoundRect(name, Color.parseColor("#2d80f8"), Utils.dpToPx(context, 14));


        Glide.with(context)
                .load(new MyGlideUrl(imageUrl))      //设置图片路径(fix #8,文件名包含%符号 无法识别和显示)
                .asBitmap()
                .centerCrop()
                .thumbnail(0.1f)
