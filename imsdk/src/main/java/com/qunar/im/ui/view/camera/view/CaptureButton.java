package com.qunar.im.ui.view.camera.view;
...
import com.qunar.im.ui.view.camera.listener.CaptureListener;
import com.qunar.im.ui.view.camera.utils.CheckPermission;

import static com.qunar.im.ui.view.camera.view.QCameraView.BUTTON_STATE_BOTH;
...
public class CaptureButton extends View {
    ...
    public CaptureButton(Context context, int size) {
        ...
        min_duration = 1000;              //默认最短录制时间为1.0s
    ...
    public boolean onTouchEvent(MotionEvent event) {
        switch (event.getAction()) {
            case MotionEvent.ACTION_DOWN:
                ...
                //判断按钮状态是否为可录制状态
                if ((button_state == QCameraView.BUTTON_STATE_ONLY_RECORDER
                        || button_state == QCameraView.BUTTON_STATE_BOTH))
                    postDelayed(longPressRunnable, 500);    //同时延长500启动长按后处理的逻辑Runnable
                break;
            case MotionEvent.ACTION_MOVE:
                if (captureLisenter != null
                        && state == STATE_RECORDERING
                        && (button_state == QCameraView.BUTTON_STATE_ONLY_RECORDER
                        || button_state == QCameraView.BUTTON_STATE_BOTH)) {
    ...
    private void handlerUnpressByState() {
        ...
        switch (state) {
            //当前是点击按下
            case STATE_PRESS:
                if (captureLisenter != null && (button_state == QCameraView.BUTTON_STATE_ONLY_CAPTURE
                        || button_state == QCameraView.BUTTON_STATE_BOTH)) {
