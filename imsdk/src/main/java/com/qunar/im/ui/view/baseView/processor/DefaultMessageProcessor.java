...
//    protected TextView statusView;
    ...
    public void processSendStatesView(TextView textView, IMessageItem item) {
        if (textView != null) {
            ...
            Log.e("IM_LOG", TAG + "processSendStatesView: --------------------");
            Log.e("IM_LOG", TAG + "processSendStatesView: messageState--" + messageState + "--" + item.getMessage().getBody());
            if (MessageStatus.isExistStatus(messageState, MessageStatus.LOCAL_STATUS_SUCCESS)) {
                int readState = item.getMessage().getReadState();

                Log.e("IM_LOG", TAG + "processSendStatesView: readState--" + readState + "--" + item.getMessage().getBody());
                if (MessageStatus.isExistStatus(readState, MessageStatus.REMOTE_STATUS_CHAT_READED)) {
//                    textView.setText(R.string.atom_ui_new_message_read); //xu
//                    textView.setTextColor(Color.parseColor("#BFBFBF"));//xu
                    textView.setText("已读");
                    textView.setTextColor(Color.parseColor("#C0C5C9"));
                } else if (MessageStatus.isExistStatus(readState, MessageStatus.REMOTE_STATUS_CHAT_DELIVERED)) {
                    textView.setText("未读");
                    textView.setTextColor(Color.parseColor("#1073FE"));
//                    textView.setText(R.string.atom_ui_new_message_unread);//xu
//                    textView.setTextColor(Color.parseColor("#00C1BA"));//xu
                } else {
                    textView.setText("未读");
                    textView.setTextColor(Color.parseColor("#1073FE"));
//                    textView.setText(R.string.atom_ui_new_message_unread);//xu
