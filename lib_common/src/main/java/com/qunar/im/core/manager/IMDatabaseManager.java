...
//                                atInfo.atcontent = content;
    ...
    public void updateDbUser(String userId, String name, String headerSrc){
        String sqlSelect = "select UserId,Name from IM_User WHERE UserId=\""+userId+"\";";
        String sqlInsert = "insert into IM_User (UserId,Name,XmppId,isVisible,HeaderSrc) VALUES ('"+userId+"','"+name+"','"+userId+ "',1,'"+headerSrc+"');";
        String sqlUpdate = "UPDATE IM_User SET Name=\""+name+"\",HeaderSrc=\""+headerSrc + "\" WHERE UserId=\""+userId+"\";";

        SQLiteDatabase db = helper.getWritableDatabase();
        if (!db.isOpen()) {
            return;
        }
        Object result = query(sqlSelect, null, new IQuery() {
    ...
    //获取会话列表数据
    public RecentConversation SelectConversation4Id(String realUser) {
        long start = System.currentTimeMillis();
        deleteJournal();
        String sql = "Select a.XmppId,a.RealJid,a.UserId,a.chatType," +
                " (case when b.type = '" + ProtoMessageOuterClass.MessageType.MessageTypeSourceCode_VALUE + "' then " + "'[代码段]'" + " else b.Content end)," +
                "b.'From',b.Type,b.ExtendedInfo,b.State," +
                ...
                "  From IM_SessionList as a Left join IM_Message as b ON a.LastMessageId = b.MsgId where a.XmppId =\""+realUser+"\" Order By top DESC, orderTime DESC;";
        SQLiteDatabase db = helper.getWritableDatabase();
        if (!db.isOpen()) {
            ...
            public Object onQuery(Cursor cursor) {
                ...
                try {
                    while (cursor.moveToNext()) {
                        ...
                        //设置置顶字段
                        rc.setTop(cursor.getInt(11));
                        //设置提醒字段
                        rc.setRemind(cursor.getInt(12));
...
//
                        ...
                        //设置最后一条消息状态
                        rc.setLastState(cursor.getString(8));
                        //扩展消息字段
                        ...
                        //获取当前会话未读消息
                        rc.setUnread_msg_cont(cursor.getInt(9));
                        //获取会话列表最后一条消息时间
                        rc.setLastMsgTime(cursor.getLong(10));
                        //许霖彧
                        Nick nick;
                        if (rc.getRealUser().contains("@conference.qtalk")) {
                            nick = ConnectionUtil.getInstance().getMucNickById(rc.getRealUser());
                        }else {
                            nick = ConnectionUtil.getInstance().getNickById(rc.getRealUser());
                        }
                        rc.setNick(nick);

                        if (rc.getNick() != null && rc.getNick().getName() != null) {
                            Log.e("IM_LOG", "onQuery: " + rc.getNick().getName());
                        }
                        rc.setLastMsg(rc.getLastMsg());
                ...
                return list.size() >= 1 ? list.get(0) : new RecentConversation();
            }
        });
        ...
        if (result == null) {
            ...
            return (RecentConversation) result;
        }
    }
    ...
    public List<RecentConversation> SelectConversationList(boolean isOnlyUnRead) {
        ...
        if (isOnlyUnRead) {
            sql = "Select a.XmppId,a.RealJid,a.UserId,a.chatType," +
                    ...
                    "(case when (select count(*) from IM_USER_CONFIG where subkey =(a.XmppId ||'<>'||a.RealJid) and isdel ='" + CacheDataType.N + "' and pkey = '" + CacheDataType.kStickJidDic + "')=1 THEN 1 ELSE 0 END )as top," +
                    "(CASE WHEN (select COUNT(*) from IM_USER_CONFIG where subkey =a.XmppId  and isdel = '" + CacheDataType.N + "' and pkey ='" + CacheDataType.kNoticeStickJidDic + "') = 1 THEN 1 ELSE 0 END) as remind  " +
                    "  From IM_SessionList as a Left join IM_Message as b ON a.LastMessageId = b.MsgId where count>0 Order By top DESC, orderTime DESC;";
...
//
    ...
    private String parsingLastMsg(String lastMsg) {
        try {
            org.json.JSONObject jsonObject = new org.json.JSONObject(lastMsg);
            if (jsonObject.has("type")) {
                String audio = jsonObject.getString("type").equals("0") ? "语音" : "视频";
                lastMsg = "[" + audio + "通话]";
            }
            if (jsonObject.has("talkType")) {
                String talkType = jsonObject.getString("talkType");
                if (talkType.equals("notConnectedMetting")) {
                    lastMsg = "[对话已拒绝]";
                } else {
                    lastMsg = "[已挂断]";
                }
            ...
            if (jsonObject.has("title") && jsonObject.has("content")) {
                lastMsg = jsonObject.getString("title");
            }
            return lastMsg;
        } catch (JSONException e) {
            if (lastMsg.contains("createMeetingVO")) {
                lastMsg = "[音视频]";
            }
            return lastMsg;
        }
...
//                imMessage.setNickName(nickName);
    ...
    public String queryMessageContent(String msgId) {
        ...
        try {
            ...
            cursor = helper.getReadableDatabase().rawQuery(sql, new String[]{msgId});
            while (cursor.moveToNext()) {
