...
public class SuperFileView extends FrameLayout implements TbsReaderView.ReaderCallback {
    ...
    private static String TAG = "TbsReader--SuperFileView";
    ...
    public void setReadFileCallback(ReadFileCallback readFileCallback) {
...
//                Looper.loop();
                    if(readFileCallback != null) readFileCallback.readFileCallback(true);
                }else {
                    if(readFileCallback != null) readFileCallback.readFileCallback(false);
                    Log.e(TAG,"文件无法预览！");
                }
            }catch (Exception e){
                Log.e(TAG, "displayFile: Exception--" + e.getMessage());
                e.printStackTrace();
            }
        } else {
            Log.e(TAG,"文件路径无效！");
